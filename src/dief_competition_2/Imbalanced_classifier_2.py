...
class Data_Prep_Pipeline:
    def __init__(self, dataset_id: str, output_dir: str, data_path: str = None) -> None:
        ...
        if data_path:
            ...
            self.data_path = (
                self.project_dir / "data" / dataset_id / f"{dataset_id}_Dataset_subhourly.csv"
            )
        ...
        if not self.daily_data_path.exists():
            self.logger.warning(f"Daily data file not found: {self.daily_data_path}")
    ...
    def get_train_test_split_specific(self, data: pl.DataFrame, train_fraction = float
    ) -> list[np.ndarray]:
        """
...
def parse_arguments(args: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data_path", type=str, default=None, help="Optional full path to data file"
    )
...
def data_prep_specific_training (args: list[str] = sys.argv):
    cfg = parse_arguments(args)
    pipeline = Data_Prep_Pipeline(cfg.dataset_id, cfg.output_dir)
    data = pipeline.get_data()
    train_X, train_Y, test_X, test_Y = pipeline.get_train_test_split_specific(data,0.4)
    return (train_X, train_Y, test_X, test_Y, cfg.dataset_id)
def data_prep_specific_testing (args: list[str] = sys.argv):
    cfg = parse_arguments(args)
    pipeline = Data_Prep_Pipeline(cfg.dataset_id, cfg.output_dir, cfg.data_path)
    data = pipeline.get_data()
    train_X, train_Y, test_X, test_Y = pipeline.get_train_test_split_specific(data,0.1)
    return (train_X, train_Y, test_X, test_Y, cfg.dataset_id)
