...
class PredictionPipeline:
    ...
    def run(self) -> None:
        data = self.get_data_for_training()
        training_data, test_data = self.get_train_test_split(data)

        test_data = self. get_data_for_testing()
    ...
    def get_data_for_training (self) -> pl.DataFrame:
        ...
        data = (
            data
            .with_columns([
                pl.col("Timestamp_Local").str.to_datetime().alias("ts"),
            ])
    ...
    def get_data_for_testing(self) -> pl.DataFrame:
        data_dir_test = pathlib.Path(
            "C:/Users/wan397/OneDrive - CSIRO/Desktop/DIEF/dief-competition-2/test/Mel"
        )

        all_dataframes = []

        for file in data_dir_test.glob("*_subhourly.csv"):  # only subhourly files
            dataset_id = file.stem.replace("_Dataset_subhourly", "")
            data = pl.read_csv(file)

            self.logger.info(f"📄 Loaded: {file.name}")
            self.logger.info(f"Test Data description for {dataset_id}:\n{data.describe()}")
            ...
            data = data.with_columns(pl.lit(dataset_id).alias("dataset_id"))
            all_dataframes.append(data)

        # Concatenate all test sets into one DataFrame
        full_test_data = pl.concat(all_dataframes, how="vertical")
    ...
    def get_train_test_split(
        self, data: pl.DataFrame, train_fraction=0.4
    ) -> list[np.ndarray]:
        """
...
if __name__ == "__main__":
    ...
    dataset_dirs = sorted([
        ...
        if f.is_dir() and not f.name.startswith(".")  # skip .gitkeep or hidden dirs
    ])

    for dataset_path in tqdm(dataset_dirs, desc="Processing datasets"):
        main([
            "--dataset_id", dataset_path.name,
            "--output_dir", f"output/lagged/{dataset_path.name}"
        ])
